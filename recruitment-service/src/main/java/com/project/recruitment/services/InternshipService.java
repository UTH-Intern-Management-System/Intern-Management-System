...
import com.project.recruitment.exceptions.DataNotFoundException;
import com.project.recruitment.exceptions.PermissionDenyException;
import com.project.recruitment.models.Benefit;
import com.project.recruitment.models.InternshipProgram;
import com.project.recruitment.models.Requirement;
import com.project.recruitment.repositories.InternshipRepository;
...
public class InternshipService {
    ...
    public InternshipResponseDTO createInternship(InternshipCreateDTO dto) throws Exception {
        UserResponse user;
        try {
            user = userClient.getUserById(dto.getCreatedBy());
        } catch (FeignException.NotFound ex) {
            throw new DataNotFoundException("User with id " + dto.getCreatedBy() + " not found");
        } catch (FeignException.Unauthorized ex) {
            throw new PermissionDenyException("Bạn không có quyền tạo Internship với user này");
        } catch (FeignException ex) {
            throw new RuntimeException("Không thể kết nối UserService: " + ex.getMessage());
        }
        ...
        user = userClient.getUserById(saved.getCreatedBy());
